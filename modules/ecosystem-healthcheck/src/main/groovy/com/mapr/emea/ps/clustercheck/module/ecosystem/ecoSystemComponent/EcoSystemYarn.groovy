...
class EcoSystemYarn {
     ...
     * @return
     */
    def verifyResourceManagerUIPam(List<Object> packages, String credentialFileREST, String certificate, Boolean useSSLCert, int port) {
        ...
        def testResult = mapRComponentHealthcheckUtil.executeSsh(packages, PACKAGE_NAME_YARN_RESOURCEMANAGER, {
            ...
            final String query = "curl -Is --netrc-file ${credentialFileREST} ${certToken} https://${remote.host}:${port}/ws/v1/cluster/info | head -n 1"
     ...
     * @return
     */
    def verifyNodeManagerUIPam(List<Object> packages, String credentialFileREST, String certificate, Boolean useSSLCert, int port) {
        ...
        def testResult = mapRComponentHealthcheckUtil.executeSsh(packages, PACKAGE_NAME_YARN_NODEMANAGER, {
            ...
            final String certToken = (useSSLCert == true) ? "--cacert ${certificate}" : "-k"
            final String query = "curl -Is --netrc-file ${credentialFileREST} ${certToken} https://${remote.host}:${port}/ws/v1/node | head -n 1"
     ...
     * @return
     */
    def verifyYarnHistoryServerPam(List<Object> packages, String credentialFileREST, String certificate, Boolean useSSLCert, int port) {
        ...
        def testResult = mapRComponentHealthcheckUtil.executeSsh(packages, PACKAGE_NAME_YARN_HISTORY_SEVER, {
            ...
            final String certToken = (useSSLCert == true) ? "--cacert ${certificate}" : "-k"
            final String query = "curl -X post -Is --netrc-file ${credentialFileREST} ${certToken} https://${remote.host}:${port}/jobhistory | head -n 1"
